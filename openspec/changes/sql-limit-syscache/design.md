...
**多节点约束**: 当前方案假设规则创建走集中式/单写管理路径。若后续迁移到多个节点均可执行 `gs_create_sql_limit_v2` 的多写环境，单节点共享原子计数不足以保证全局唯一：不同节点可能在相同 `max(limit_id)` 基础上生成相同 ID。多写场景必须替换为全局 ID 分配机制，例如全局 sequence、GTM/协调节点分配，或带节点号分段的 ID 方案。

**替代方案**: 使用系统 sequence 生成 `limit_id`。优点是持久化、并发安全、重启后无需扫描规则表；缺点是新增系统 sequence 会引入额外 OID、升级回滚、shared catalog 可见性和依赖管理成本。集中式实现先采用共享原子计数，迁移到多写环境时重新评估。
